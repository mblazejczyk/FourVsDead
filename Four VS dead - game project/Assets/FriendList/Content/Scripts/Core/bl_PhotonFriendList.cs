...
public class bl_PhotonFriendList : MonoBehaviourPunCallbacks
{
    ...
    void ClearUI()
    {
        ...
        foreach(GameObject ob in GameObject.FindGameObjectsWithTag("FriendUiElement"))
        {
            Destroy(ob);
        }
    }
    ...
    {
        if (Content.GetComponent<Animator>().GetBool("Close"))
        {
            Content.GetComponent<Animator>().SetBool("Close", false);
        }
        else
        {
            Content.GetComponent<Animator>().SetBool("Close", true);
        }
    }
    #endregion

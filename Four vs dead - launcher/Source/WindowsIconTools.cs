...
{
	// https://yellowafterlife.itch.io/window-set-icon

	/// <summary>
	/// A library for displaying custom information on the taskbar icon, such as download progress.
	/// </summary>
				...
				if(!_taskbarListReady)
				{
					lock(_initLock)
					{
						if(!_taskbarListReady)
						{
							...
							{
								Console.WriteLine("ITaskbarList4 init failed! Make sure you are using .NET Core 4.x");
							}
